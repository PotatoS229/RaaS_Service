...
```
flowchart TD
    A["Клиентское приложение\n(например, онлайн-игра)"] --> B["udp2raw-client\n(Локальный UDP-слушатель)\nПОРТ: 3333"]
    B --> C["udp2raw-client\n(Ядро: Обработка пакета)"]
    
    C --> D{"Выбор режима\n(raw-mode)"}
    D -- "faketcp/icmp" --> E["Шифрование и\nAnti-Replay\n(AES, HMAC)"]
    E --> F["Инкапсуляция\nв 'обертку' TCP/ICMP\n(Сырой сокет)"]
    F --> G["Пересылка по Сети\n(Замаскированный трафик)"]
    
    D -- "udp" --> H["Шифрование и\nAnti-Replay\n(AES, HMAC)"]
    H --> I["Инкапсуляция\nв обычный UDP\n(Сырой сокет)"]
    I --> G
    
    G --> J["Межсетевой экран/Провайдер\n(Видит только TCP/ICMP/UDP)"]
    J --> K["udp2raw-server\n(Сырой сокет)"]
    
    K --> L["Деинкапсуляция\n(Снятие 'обертки')"]
    L --> M["Расшифровка и\nпроверка целостности"]
    M --> N["udp2raw-server\n(Пересылка на целевой порт)"]
    N --> O["Целевой UDP-сервис\n(например, игровой сервер)\nПОРТ: 7777"]